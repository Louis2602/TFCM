...
export function DocumentCollection() {
  ...
  const onMoveContentToCategory = (content: Content, category: Category) => {
    moveContentToCategory(content.id, category.id)
      .then((res) => {
        if (res.success) {
          toast.success(content.title + " has been moved to " + category.name);
        } else {
          toast.error("Unable to move content to category: " + res.error);
        }
      })
      .catch((err) => {
    ...
    removeContentCategory(content.id)
      .then((res) => {
        if (res.success) {
          ...
          toast.error("Unable to remove content from category: " + res.error);
        }
      })
      .catch((err) => {
        toast.error("Unable to update category: " + err);
